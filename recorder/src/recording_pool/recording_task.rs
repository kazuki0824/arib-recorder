...
machine!(
    ...
    pub(crate) enum RecordingState {
        ...
        Rec { since: DateTime<Local>, end: DateTime<Local> },
        Timeout {},
        Success {},
    }
);
...
impl RecordingTask {
    pub(crate) async fn new(info: &RecordingTaskDescription) -> Result<Self, Error> {
        ...
        let target = Some(IoObject::new(Some(file_location.as_path())).await?);
        Ok(Self {
            target,
            eit: EitParser::new(),
            ...
            *me.next_state = after;
            ...
            if me.state != me.next_state {
                ...
                let new_path = match me.next_state {
                    RecordingState::Rec(_) if me.file_location.set_extension("m2ts") => Some(me.file_location.as_path()),
                    RecordingState::B1(_) | RecordingState::B2(_) if me.file_location.set_extension("m2ts-tmp") => Some(me.file_location.as_path()),
                    _ => None,
                };
